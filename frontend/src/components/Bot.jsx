...
import { Mic, Square, Volume2, VolumeX, Video, VideoOff } from "lucide-react";
...
const Bot = () => {
  ...
  const [emotionDetectorActive, setEmotionDetectorActive] = useState(true);
  ...
  const handleEmotionDetected = async (imageData) => {
    try {
      // Call the backend to analyze the emotion
      ...
      // Update the emotion in the EmotionDetector component
      if (emotionDetectorRef.current) {
        emotionDetectorRef.current.updateEmotion(data.emotion);
      }
    } catch (err) {
  ...
  const toggleEmotionDetector = () => {
    setEmotionDetectorActive(prev => !prev);
  };
  ...
  const handleSendMessage = async (voiceText = null) => {
    ...
    try {
      ...
      // Trigger a facial expression capture before sending message
      if (emotionDetectorActive) {
        // Force a capture now for the current message
        if (emotionDetectorRef.current) {
          emotionDetectorRef.current.triggerCapture();
        }
      }
      ...
      const response = await fetch("http://localhost:8000/chat/", {
        ...
        body: JSON.stringify({ 
          text: messageToSend,
          image_data: emotionDetectorActive ? await getCurrentFrame() : null 
        }),
      });
      ...
      const botResponse = {
        ...
        timestamp: new Date(),
        detectedEmotion: data.detected_emotion || null,
      };
  ...
  const getCurrentFrame = async () => {
    if (!emotionDetectorActive) return null;
  ...
  return (
    ...
    <NavBar />
      <div className="flex flex-col items-center min-h-[90vh] bg-gradient-to-br from-blue-100 to-purple-200 dark:bg-gradient-to-br dark:from-gray-800 dark:to-gray-950 pb-20">
        ...
        {/* Emotion detector toggle and display */}
        <div className="absolute top-20 right-4 flex flex-col items-end gap-2">
          <button 
            onClick={toggleEmotionDetector}
            className="bg-white/80 dark:bg-gray-800/80 p-2 rounded-full shadow-md hover:bg-white dark:hover:bg-gray-700"
            title={emotionDetectorActive ? "Disable emotion detection" : "Enable emotion detection"}
          >
            {emotionDetectorActive ? <VideoOff size={20} /> : <Video size={20} />}
          </button>
          
          {emotionDetectorActive && (
            <EmotionDetector 
              ref={emotionDetectorRef}
              onEmotionDetected={handleEmotionDetected} 
              isActive={emotionDetectorActive} 
            />
          )}
        </div>
              ...
              >
                <div className="flex justify-between items-center mb-1">
                  <div className="flex items-center gap-2">
                    ...
                    {msg.emotion && (
                      <span title={`Detected: ${msg.emotion.emotion}`} className="text-sm">
